...
to decide-road-and-shelter-route [idxrep] ;* tambahan untuk generate path pada saat Setup
...
to follow-predefined-path ;* tambahan untuk agen saat mengikuti path yg didapat saat setup
  ...
  if (ticks >= (td * 60)) [ ;jika td masuk tick, baru mulai jalan
    ...
    [ ifelse not empty? path
      [ let next first path
        ...
        if patch-here = next
         [ 
           ;pengguncangan keteguhan agen, mulai.
...
to-report adjust-speed ; * adjust speed dimodifikasi bagian 5^2 karena scalenya beda dengan Mas Erick
  let s 0
  ...
  let p count pedestrians ;hitung jumlah pejalan kaki, tanpa mempedulikan waktu keberangkatannya (modif dari mas Erick
...
;; modified June 2012
;; revised July 2012
;; modified again February 2014 by Wafdan
;;*******************************************************************
to-report Astar [setup-start setup-goal setup-color behavior]
  ...
  ask start [set open? 1 set parent self 
    if space-color = green and pcolor != green [ set pcolor green ]] ;* modifikasi untuk memastikan patch yang "diinjak" oleh agen tidak ambigu

  while [ [open?] of target != 2 ]
  [
    if count patches with [open? = 1] = 0 and [open?] of start != 1 [report [ ]]
...
to-report new-path-cost [ current candidate ]  ;asli dari Mas Erick
  ask candidate [ set parent current fgh-values ]
  let cost path-cost candidate
  report cost
end

to-report show-path ;asli dari Mas Erick
  let brush target
  let my-path [ ]
...
to-report #heuristic ;heuristik murni dari Mas Erick
